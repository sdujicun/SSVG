 ...
 */
public class ShapeletTransformWithHVGNoSample extends SimpleBatchFilter {
	 ...
	 */
	public ShapeletTransformWithHVGNoSample() {
		this(DEFAULT_NUMSHAPELETS, DEFAULT_MINSHAPELETLENGTH,
				DEFAULT_MAXSHAPELETLENGTH,
	 ...
	 * @param shapes
	 */
	public ShapeletTransformWithHVGNoSample(ArrayList<Shapelet> shapes) {
		this();
	 ...
	 */
	public ShapeletTransformWithHVGNoSample(int k) {
	 ...
	 */
	public ShapeletTransformWithHVGNoSample(int k, int minShapeletLength,
			int maxShapeletLength) {
		this(k, minShapeletLength, maxShapeletLength,
				QualityMeasures.ShapeletQualityChoice.INFORMATION_GAIN);
	...
	protected void initQualityBound(ClassDistribution classDist) {
		...
		switch (qualityChoice) {
		case F_STAT:
			this.qualityBound = new QualityBound.FStatBound(classDist,
					candidatePruningStartPercentage);
			break;
		case KRUSKALL_WALLIS:
			this.qualityBound = new QualityBound.KruskalWallisBound(classDist,
					candidatePruningStartPercentage);
			break;
		case MOODS_MEDIAN:
			this.qualityBound = new QualityBound.MoodsMedianBound(classDist,
					candidatePruningStartPercentage);
			break;
		default:
			this.qualityBound = new QualityBound.InformationGainBound(
					classDist, candidatePruningStartPercentage);
		}
	}
	...
	protected Instances determineOutputFormat(Instances inputFormat)
		...
		if (inputFormat.classIndex() >= 0) {
			...
			atts.addElement(new Attribute(inputFormat.attribute(
					inputFormat.classIndex()).name(), vals));
		}
		Instances result = new Instances("Shapelets"
				+ inputFormat.relationName(), atts, inputFormat.numInstances());
		if (inputFormat.classIndex() >= 0) {
	...
	protected void inputCheck(Instances dataInst)
			throws IllegalArgumentException {
		if (numShapelets < 1) {
			throw new IllegalArgumentException(
	...
	protected void trainShapelets(Instances data) {
		...
		shapelets = findBestKShapeletsCacheWithHVG(dataset); // get k
																// shapelets
	...
	protected Instances buildTansformedDataset(Instances data,
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < dataSize; j++) {
				dist = subseqDistance.calculate(data.instance(j)
						.toDoubleArray(), j);
		...
		for (int j = 0; j < dataSize; j++) {
			...
			// binarised or not.
			output.instance(j).setValue(size,
					classValue.getUnAlteredClassValue(data.instance(j)));
		}
		return output;
	}

	public ArrayList<Shapelet> findBestKShapeletsCacheWithHVG(Instances data) {
		...
		for (; dataSet < dataSize; dataSet++) {
			...
			worstShapelet = kShapelets.size() == numShapelets ? kShapelets
					.get(numShapelets - 1) : null;
			...
			seriesShapelets = searchFunction.SearchForShapeletsInSeriesWithHVG(
					data.get(dataSet), this::checkCandidate, this.minDiff);
	...
	public ArrayList<Shapelet> findBestKShapeletsCacheWithHVG(int numShapelets,
			Instances data) {
		...
		subseqDistance.init(data);
		initDataSouce(data);
		return findBestKShapeletsCacheWithHVG(data);
	}
	 ...
	 *         ArrayLists
	 */
	protected ArrayList<Shapelet> combine(int k,
			ArrayList<Shapelet> kBestSoFar,
	 ...
	 */
	protected static ArrayList<Shapelet> removeSelfSimilar(
			ArrayList<Shapelet> shapelets) {
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = i + 1; j < size; j++) {
				...
				if ((!selfSimilar[j])
						&& selfSimilarity(shapelets.get(i), shapelets.get(j))) {
					selfSimilar[j] = true;
				}
			}
		}
	...
	protected Shapelet checkCandidate(double[] series, int start, int length) {
		...
		for (int i = 0; i < dataSize; i++) {
			...
			if (i != dataSet) {
				distance = subseqDistance.calculate(inputData.instance(i)
						.toDoubleArray(), i);
			}
			...
			if (qualityBound != null) {
				qualityBound
						.updateOrderLine(orderline.get(orderline.size() - 1));
			}
		}

		Shapelet shapelet = new Shapelet(candidate, dataSourceIDs[dataSet],
				start, this.qualityMeasure);
		...
		if (candidate.seriesId == shapelet.seriesId) {
			if (candidate.startPos >= shapelet.startPos
					&& candidate.startPos < shapelet.startPos
							+ shapelet.content.length) { // exisiting
															// //
															// shapelet
				return true;
			}
			if (shapelet.startPos >= candidate.startPos
					&& shapelet.startPos < candidate.startPos
	 ...
	 * @throws Exception
	 */
	public static ShapeletTransformWithHVGNoSample createFilterFromFile(
	...
	public double timingForSingleShapeletBasedOnLFDP(Instances data) {
		...
		findBestKShapeletsCacheWithHVG(1, data);
	...
	protected void recordShapelets(ArrayList<Shapelet> kShapelets,
		...
		try {
			...
			for (Shapelet kShapelet : kShapelets) {
				out.append(kShapelet.qualityValue + "," + kShapelet.seriesId
						+ "," + kShapelet.startPos + "\n");
	...
	protected void printShapelets(ArrayList<Shapelet> kShapelets) {
		...
		System.out.println();
		for (Shapelet kShapelet : kShapelets) {
			System.out.println(kShapelet.qualityValue + ","
					+ kShapelet.seriesId + "," + kShapelet.startPos + ","
					+ kShapelet.classValue);
			double[] shapeletContent = kShapelet.content;
			for (int j = 0; j < shapeletContent.length; j++) {
	 ...
	 */
	public static ShapeletTransformWithHVGNoSample createFilterFromFile(
			String fileName, int maxShapelets) throws Exception {

		File input = new File(fileName);
		Scanner scan = new Scanner(input);
		scan.useDelimiter("\n");

		ShapeletTransformWithHVGNoSample sf = new ShapeletTransformWithHVGNoSample();
	...
	public static Instances roundRobinData(Instances data, int[] sourcePos) {
		...
		for (int i = 0; i < dataSize;) {
			...
			for (int j = 0; j < classDistribution.size(); j++) {
				ArrayList<Instance> currentList = instancesByClass
						.get((double) j);
				ArrayList<Integer> currentPositions = positionsByClass
						.get((double) j);

				if (!currentList.isEmpty()) {
					roundRobinData
							.add(currentList.remove(currentList.size() - 1));
					if (sourcePos != null && sourcePos.length == dataSize) {
						sourcePos[i] = currentPositions.remove(currentPositions
								.size() - 1);
					}
					i++;
				}
	 ...
	 * @throws Exception
	 */
	public long opCountForSingleShapeletWithHVG(Instances data)
			throws Exception {
		data = roundRobinData(data, null);
		subseqDistOpCount = 0;
		findBestKShapeletsCacheWithHVG(1, data);
	...
	public static void trainTestExample(String problem) {
		try {
			...
			Logger.getLogger(ShapeletTransformWithHVGNoSample.class.getName())

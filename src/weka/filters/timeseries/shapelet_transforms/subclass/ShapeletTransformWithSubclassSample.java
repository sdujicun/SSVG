 ...
 */
public class ShapeletTransformWithSubclassSample extends SimpleBatchFilter {
	 ...
	 */
	public ShapeletTransformWithSubclassSample() {
	 ...
	 * @param shapes
	 */
	public ShapeletTransformWithSubclassSample(ArrayList<Shapelet> shapes) {
		this();
	 ...
	 */
	public ShapeletTransformWithSubclassSample(int k) {
	 ...
	 */
	public ShapeletTransformWithSubclassSample(int k, int minShapeletLength, int maxShapeletLength) {
	 ...
	 */
	public ShapeletTransformWithSubclassSample(int k, int minShapeletLength, int maxShapeletLength, weka.core.shapelet.QualityMeasures.ShapeletQualityChoice qualityChoice) {
	...
	public ArrayList<Shapelet> findBestKShapeletsCache(Instances data) {
		...
		// // full.
		// worstShapelet = kShapelets.size() == numShapelets ?
		// kShapelets.get(numShapelets - 1) : null;
		//
		...
		//
		// seriesShapelets =
		// searchFunction.SearchForShapeletsInSeries(data.get(dataSet),
		// this::checkCandidate);
		//
		...
		//edit by jc 
		//for selected index
		List<Integer> list = new SubclassSample(). subClassSplitting(data);
		for (int i = 0; i < list.size(); i++) {
			// System.out.println(list.get(i));
			worstShapelet = kShapelets.size() == numShapelets ? kShapelets.get(numShapelets - 1) : null;
			// set the series we're working with.
			subseqDistance.setSeries(list.get(i));
			// set the clas value of the series we're working with.
			classValue.setShapeletValue(data.get(list.get(i)));

			seriesShapelets = searchFunction.SearchForShapeletsInSeries(data.get(list.get(i)), this::checkCandidate);
	...
	private static boolean selfSimilarity(Shapelet shapelet, Shapelet candidate) {
		if (candidate.seriesId == shapelet.seriesId) {
			if (candidate.startPos >= shapelet.startPos && candidate.startPos < shapelet.startPos + shapelet.content.length) { // candidate
																																// starts
																																// within
																																// exisiting
																																// shapelet
	 ...
	 * @throws Exception
	 */
	public static ShapeletTransformWithSubclassSample createFilterFromFile(String fileName) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static ShapeletTransformWithSubclassSample createFilterFromFile(String fileName, int maxShapelets) throws Exception {

		File input = new File(fileName);
		Scanner scan = new Scanner(input);
		scan.useDelimiter("\n");

		ShapeletTransformWithSubclassSample sf = new ShapeletTransformWithSubclassSample();
	...
	public static void main(String[] args) {
		String[] problems = { "Car","ChlorineConcentration", "Coffee", "ECGFiveDays", "GunPoint", "Lightning7", "MedicalImages", "MoteStrain", "Trace", "TwoLeadECG" };
...
//			System.out.println(accuracy);
			...
			Logger.getLogger(ShapeletTransformWithSubclassSample.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
